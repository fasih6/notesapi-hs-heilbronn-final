...
credentials.token = getenv("GOOGLE_AUTH_ACCESS_TOKEN", "")
request = Request()

try:
    # Initialize Cloud Trace with the access token
    from opentelemetry.exporter.cloud_trace import CloudTraceSpanExporter
    cloud_trace_exporter = CloudTraceSpanExporter(credentials=credentials, request=request)
    span_processor = BatchSpanProcessor(cloud_trace_exporter)
    trace.get_tracer_provider().add_span_processor(span_processor)
except Exception as e:
    print(f"Error initializing Cloud Trace: {e}")
